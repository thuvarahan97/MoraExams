...
if (mysqli_num_rows($results_check)>0) {
    ...
    if($result_checked) {
        redirect("student.details.php?success=checked");
    }
    else {
        redirect('students.details.php?error='.md5("check_failed"));
    }
}
else {
    redirect('students.details.php?error='.md5("not_found_error"));
}
